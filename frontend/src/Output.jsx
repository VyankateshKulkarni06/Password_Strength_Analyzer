...
const Output = () => {
  ...
  // Better data handling with proper localStorage serialization
  const data = useMemo(() => {
    // First try to get data from location state
    const locationData = location.state?.data;
    if (locationData) {
      // Store in localStorage when we get new data from location
      try {
        localStorage.setItem("passwordData", JSON.stringify(locationData));
      } catch (e) {
        console.error("Failed to store password data", e);
      }
      return locationData;
    }
    ...
    try {
      ...
      console.error("Failed to parse stored password data", e);
    }
  ...
  // Generate matrix characters for the background
  useEffect(() => {
    const chars = [];
    for (let i = 0; i < 15; i++) {
      const lineChars = [];
      for (let j = 0; j < 15; j++) {
        lineChars.push(String.fromCharCode(33 + Math.floor(Math.random() * 93)));
      }
      chars.push({
        id: i,
  ...
  // Matrix background with Tailwind classes
  const renderMatrixBackground = () => (
    <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none opacity-20 z-0">
      {matrixChars.map((line) => (
        <div 
          key={line.id}
          className="absolute top-0 flex flex-col animate-matrix-drop" 
          style={{
            left: line.left,
        ...
        >
          {line.chars.map((char, idx) => (
            <span 
              key={idx} 
              className="text-green-700 animate-flicker"
              style={{
    ...
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-black p-4">
        <div className="w-full max-w-3xl bg-gray-900 text-white p-6 rounded-lg shadow-lg font-mono border border-gray-800">
          <p>No data available. Please return to the password input screen.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-black p-4">
      <div className="w-full max-w-3xl bg-black text-green-500 p-6 rounded-lg shadow-lg font-mono border border-green-900 relative">
        ...
        {/* Tailwind styled matrix background */}
        {renderMatrixBackground()}
      </div>

      {/* Animation keyframes for matrix effect */}
      <style jsx>{`
